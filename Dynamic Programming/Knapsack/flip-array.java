...
the elements of array should be minimum non-negative(as close to zero as possible). Return the minimum no. of elements whose
sign needs to be flipped such that the resultant sum is minimum non-negative.
...
public class Solution {
  ...
  int rec(int i, int j, List<Integer> A) {
    ...
    if (w > j) {
      ...
      int wnum = map.get(wKey) != null ? map.get(wKey) + 1 : 1;

      String prevKey = String.format("%d_%d", i - 1, j);
      //Set<Integer> prevSet = map.get(prevKey);
      //int prevnum = prevSet == null ? 0 : prevSet.size();
      int prevnum = map.get(prevKey) != null ? map.get(prevKey) : 0;
